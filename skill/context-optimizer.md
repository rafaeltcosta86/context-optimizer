...
**Purpose:** Run the tool calls Phase 2 needs to analyze the project.

**First action (mandatory):** Before any tool calls, output exactly this line to the user:
*Scanning project structure...*

**Tool calls (run all that apply, in order):**

1.  Glob for these files and directories (no analysis — just locate them):
    *   **Claude Code:** `CLAUDE.md`, `.claude/settings.json`, `.claude/settings.local.json`, `.claude/hooks/*`, `~/.claude/projects/<proj>/memory/MEMORY.md`
    ...
    *   **Global user-level:** `~/.claude/settings.json`, `~/.claude/hooks/session-start.sh`
    *   **Reference files:** `*.md` files outside the agent config globs above (`docs/*.md`, `CONTRIBUTING.md`, `README.md`).
2.  Read each detected agent context file in full. Read the first 30 lines of `README.md`. Read the first found among: `package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, `mix.exs`.
3.  Run `test -e .git`. If it passes: run `git rev-parse --abbrev-ref HEAD`, `git log --oneline -5`, `git status --short`. If `gh auth status` passes: run `gh issue list --state open --json number,title --limit 5` and `gh pr list --state open --json number,title,headRefName --limit 5`.

**Phase 1 produces no user-visible output other than the status line above.** All tool-call results stay in context for Phase 2 to consume — Phase 1 does not analyze, summarize, score, or render them.

**Transition (mandatory):** Output `---DIAGNOSIS-REPORT-START---` on the line immediately after the status line — no other text, no blank line. Begin Phase 2 content on the line after `---DIAGNOSIS-REPORT-START---`.
...
6.  **Compute scores.** Two parallel computations from Phase 1 tool-call results and Steps 1–5 outputs:
    *   **Context Health Score** — apply the `score-health-computation` Known Pattern:
    ...
    *   **Stage Signals** — apply the multi-signal heuristic to Phase 1 tool-call results:
        ...
        - **Weighted score rule:** Total weighted score ≥ 4 → recommend stage contracts (consumed by Phase 4). Weights: strong=3pts, medium=2pts, weak=1pt.
        - Also record raw signal count (count of distinct signals detected, ignoring weights) — feeds Phase 3 weak-state check.
